...
def list_docs():
    ...
    try:
        # 步骤 1: 从 Document 表获取文档的基础信息列表。
        # Document 表存储的是文档的"宏观状态"，例如它属于哪个知识库、基本信息等。
        docs, tol = DocumentService.get_by_kb_id(
            kb_id, page_number, items_per_page, orderby, desc, keywords)

        # 步骤 2: 遍历每个文档，进行"二次处理"以获取最实时的状态。
        for doc_item in docs:
            ...
            # 步骤 3: 查询关联的 Task 记录。
            # Task 表是任务执行的"事实真相源(Source of Truth)"，记录了最详细、最实时的进度。
            # 无论 Document 表中的状态是什么，我们都优先采信 Task 表的信息。
            tasks = TaskService.query(doc_id=doc_item["id"])
            if tasks:
                # 如果找到了对应的 Task 记录
                task = tasks[0]
                # 用 Task 表中的实时进度，覆盖从 Document 表中读出的可能过时的进度。
                doc_item["progress"] = task.progress
                # 用 Task 表中的详细进度消息，覆盖 Document 表中的概要信息。
                doc_item["progress_msg"] = task.progress_msg
                
                # 步骤 4: 添加保险措施，确保前端能立即看到"成功"状态。
                # 如果任务进度已达100%，就直接在返回的数据中将文档状态设置为"DONE"。
                # 这避免了因数据库更新延迟，导致前端看到进度条满了但状态仍是"运行中"的尴尬情况。
                if task.progress == 1.0 :
                    doc_item["run"] = TaskStatus.DONE.value
...
def parse():
   ...
   #TODO: 修改了parse接口图片处理的逻辑，会导致api接口调用parse接口时，行为不一致，编辑器中试运行时会调用parse
   # 但是api接口调用时，不会调用parse接口，而是调用upload_and_parse接口
   # 同时，parse接口还被解析文件的sdk调用，这导致sdk调用parse接口时，行为不一致

    # 分离图片文件和其他文件
    image_files = []
    other_files = []
    for file in file_objs:
        if filename_type(file.filename) == FileType.VISUAL.value:
    ...
    for img_file in image_files:
        try:
            ...
            ext = pathlib.Path(img_file.filename).suffix[1:].lower()
            img_tag = f"data:image/{ext};base64,{base64_str}"
            image_contents.append(img_tag)
        except Exception as e:
            logging.error(f"Failed to process image {img_file.filename}: {str(e)}")

    # 处理其他文件（保持原有逻辑）
    other_content = FileService.parse_docs(other_files, current_user.id) if other_files else ""
