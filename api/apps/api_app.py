...
def retrieval():
    ...
    try:
        ...
        if not kbs:
            return get_json_result(
                data=False, message='No knowledge bases found.',
                code=settings.RetCode.DATA_ERROR)

        # 2. 检查所有知识库是否使用相同的 embedding model
        ...
        # 3. 收集所有知识库的 tenant_ids
        tenant_ids = list(set([kb.tenant_id for kb in kbs]))
        
        # 4. 使用第一个知识库的 embedding model（因为已经确认所有知识库使用相同的模型）
        embd_mdl = TenantLLMService.model_instance(
            kbs[0].tenant_id, LLMType.EMBEDDING.value, llm_name=kbs[0].embd_id)
        
        # 5. 设置 rerank model（如果有）
        rerank_mdl = None
