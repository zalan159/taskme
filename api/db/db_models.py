...
class Tenant(DataBaseModel):
    ...
    mcp_id = CharField(
        max_length=256,
        null=True,
        help_text="default mcp model ID",
        index=True)
...
class CanvasInfo(Model):
    canvas_id = CharField(max_length=32, primary_key=True)
    canvas_tag = JSONField(null=True)
    canvas_permissions = CharField(max_length=32, null=False)
    canvas_industry = CharField(max_length=32, null=True)
    icon = CharField(max_length=32, null=True)
    description = CharField(max_length=255, null=True)
    prompts = JSONField(null=True)
    is_recommended = BooleanField(default=False, null=True, help_text="Is this canvas/agent recommended?")

    class Meta:
        database = DB
        db_table = "canvas_info"

class CanvasTenant(Model):
    id = CharField(max_length=32, primary_key=True)
    canvas_id = CharField(max_length=32, null=False)
    tenant_id = CharField(max_length=32, null=False)

    class Meta:
        database = DB
        db_table = "canvas_tenant"
...
class ChatBot(Model):
    ...
    client_secret = CharField(max_length=255, null=False)
    user_id = CharField(max_length=255, null=False)
    canvas_id = CharField(max_length=32, null=False)
    status = CharField(max_length=20, null=False, default="stop")
...
# 聊天信息表
class ChatBotConversation(Model):
    id = CharField(max_length=32, primary_key=True)
    client_id = CharField(max_length=32, null=False)
    user_id = CharField(max_length=255, null=False)
    message = JSONField(null=True)
...
def migrate_db():
    with DB.transaction():
        ...
        try:
            migrate(
                migrator.add_column('canvas_info', 'is_recommended', BooleanField(default=False, null=True, help_text="Is this canvas/agent recommended?"))
            )
        except Exception:
            pass      
