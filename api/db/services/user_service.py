...
class TenantService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def get_mcp_credential(cls, tenant_id):
        """
        Get MCP credential for a tenant.
        """
        from api.db.services.llm_service import LLMService
        success, tenant = cls.get_by_id(tenant_id)
        if not success or not tenant.mcp_id:
            raise ValueError("Tenant or mcp_id not configured.")

        model_name, provider = tenant.mcp_id.split("@")

        llm = LLMService.get_by_provider(tenant_id, provider)
        if not llm or not llm.api_key:
            raise ValueError(f"API key for provider '{provider}' not found for the tenant.")
