...
class LLMService(CommonService):
    ...
    def get_by_provider(cls, tenant_id, provider):
        """
        Get LLM info by provider for a specific tenant.
        If api_base is not set in DB, use the default one.
        """
        objs = TenantLLM.select().where(
            (TenantLLM.tenant_id == tenant_id) &
            (TenantLLM.llm_factory == provider)
        ).first()

        if objs and not objs.api_base:
            provider_key = provider.lower()
            if provider_key in cls._DEFAULT_BASE_URLS:
                objs.api_base = cls._DEFAULT_BASE_URLS[provider_key]
...
class TenantLLMService(CommonService):
    ...
    def get_model_config(cls, tenant_id, llm_type, llm_name=None):
        ...
        elif llm_type == LLMType.MCP:
            mdlnm = tenant.mcp_id if not llm_name else llm_name
        else:
    ...
    def increase_usage(cls, tenant_id, llm_type, used_tokens, llm_name=None):
        ...
        llm_map = {
            ...
            LLMType.MCP.value: tenant.mcp_id if not llm_name else llm_name
        }
...
class LLMBundle(object):
    ...
    def cv_with_prompt(self, image, prompt, max_tokens=300):
        """
        使用自定义prompt处理图像输入
        
        Args:
            image: 图像数据（base64编码或二进制）
            prompt: 自定义prompt
        ...
        """
        # 打印参数以便调试
        print(f"LLMBundle.cv_with_prompt 参数: image_len={len(image) if image else 0}, prompt='{prompt}', max_tokens={max_tokens}")
        
        # 检查并修正max_tokens，确保值在合理范围内
        if max_tokens is None or max_tokens < 1:
            max_tokens = 1000
        elif max_tokens > 8192:
            max_tokens = 8192
            
        # 检查模型是否支持cv_with_prompt方法
        if not hasattr(self.mdl, 'cv_with_prompt'):
            # 如果模型不支持cv_with_prompt方法，则使用默认的describe方法
            print(f"模型不支持cv_with_prompt方法，使用describe替代，max_tokens={max_tokens}")
            txt, used_tokens = self.mdl.describe(image, max_tokens)
            if not TenantLLMService.increase_usage(
                    self.tenant_id, self.llm_type, used_tokens):
                logging.error(
                    "LLMBundle.cv_with_prompt can't update token usage for {}/IMAGE2TEXT used_tokens: {}".format(self.tenant_id, used_tokens))
            return txt
            
        # 使用模型的cv_with_prompt方法
        print(f"调用模型的cv_with_prompt方法，max_tokens={max_tokens}")
        txt, used_tokens = self.mdl.cv_with_prompt(image, prompt, max_tokens)
        if not TenantLLMService.increase_usage(
                self.tenant_id, self.llm_type, used_tokens):
            logging.error(
                "LLMBundle.cv_with_prompt can't update token usage for {}/IMAGE2TEXT used_tokens: {}".format(self.tenant_id, used_tokens))
