...
class QWenCV(Base):
    ...
    def cv_with_prompt(self, image, prompt, max_tokens=1000):
        print(f"QWenCV.cv_with_prompt: image长度={len(image) if image else 0}, prompt='{prompt}', max_tokens={max_tokens}")
        """
        使用自定义prompt处理图像输入
        ...
        # 处理max_tokens为None的情况
        if max_tokens is None:
            max_tokens = 1000  # 设置默认值
        # 确保max_tokens在有效范围内
        if max_tokens < 1:
            max_tokens = 1
        ...
        # 打印最终使用的max_tokens值
        print(f"QWenCV.cv_with_prompt 最终使用的max_tokens值: {max_tokens}")
        ...
        try:
            # 打印请求参数
            print(f"调用MultiModalConversation.call, model={self.model_name}, max_tokens={max_tokens}")
            
            response = MultiModalConversation.call(
                model=self.model_name,
                messages=messages,
                max_tokens=int(max_tokens)  # 确保转换为整数
            )
            if response.status_code == HTTPStatus.OK:
                result_text = response.output.choices[0]['message']['content'][0]["text"]
                print(f"请求成功，响应文本长度: {len(result_text)}, tokens: {response.usage.output_tokens}")
                return result_text, response.usage.output_tokens
...
class OpenAI_APICV(GptV4):
    ...
    def cv_with_prompt(self, image, prompt, max_tokens=300):
        ...
        Returns:
            生成的文本描述和使用的token数
        """
        print(f"OpenAI_APICV.cv_with_prompt: image长度={len(image) if image else 0}, prompt='{prompt}', max_tokens={max_tokens}")
        ...
        print(f"OpenAI_APICV.cv_with_prompt 最终使用的max_tokens值: {max_tokens}")
        ...
        try:
            print(f"调用OpenAI API, model={self.model_name}, max_tokens={max_tokens}")
            ...
            print(f"请求成功，响应文本长度: {len(result_text)}, tokens: {response.usage.total_tokens}")
            return result_text, response.usage.total_tokens
        except Exception as e:
            print(f"请求异常: {str(e)}")
            return f"**ERROR**: {str(e)}", 0
