import { WeChatIcon } from '@/assets/icon/Icon';
...
const Login = () => {
  ...
  const toWechat = () => {
    const { wechat } = appConf;
    if (!wechat) return;

    const encodedRedirectUri = encodeURIComponent(wechat.redirectUri);
    const wechat_url = `https://open.weixin.qq.com/connect/qrconnect?appid=${wechat.appId}&redirect_uri=${encodedRedirectUri}&response_type=code&scope=snsapi_login#wechat_redirect`;
    console.log('准备跳转微信登录：', wechat_url);
