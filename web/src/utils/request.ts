...
request.interceptors.response.use(async (response: Response, options) => {
  ...
  // Ignore 109 error code completely (No authorization)
  if (data?.code === 109) {
    return response;
  }

  // Only suppress KB API error notifications for read operations (GET requests)
  // and specific view-only API paths, but keep error notifications for actions like delete
  const url = response.url || '';
  const method = options?.method?.toUpperCase() || '';
  const isReadOperation = method === 'GET';
  const isViewOnlyKbPath =
    url.includes('/v1/kb/detail') ||
    url.includes('/v1/kb/knowledge_graph') ||
    url.includes('/v1/kb/tags');

  if (
    (isReadOperation || isViewOnlyKbPath) &&
    url.includes('/v1/kb/') &&
    data?.code !== 0
  ) {
    console.log(
      `Suppressing KB API error notification for ${url} with code ${data?.code}`,
    );
    return response;
  }
