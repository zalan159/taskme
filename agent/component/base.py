    ...
    def as_dict(self):
        def _recursive_convert_obj_to_dict(obj):
            ...
            if obj is None or isinstance(obj, (int, float, str, bool)):
                ...
                return {str(k): _recursive_convert_obj_to_dict(v) for k, v in obj.items()}
                ...
                for attr_name in list(obj.__dict__):
                    ...
                    # 跳过函数、方法、生成器等不可序列化对象
                    if callable(attr) or inspect.isgenerator(attr) or inspect.isasyncgen(attr):
                        continue
                    ret_dict[attr_name] = _recursive_convert_obj_to_dict(attr)
                return ret_dict
            elif inspect.isgenerator(obj) or inspect.isasyncgen(obj):
                # 安全处理生成器
                return {"_type": "generator", "value": "<generator object>"}
...
class ComponentBase(ABC):
    ...
    @staticmethod
    def be_output(v, is_error=True):
        """
        将输入转换为输出格式
        
        Args:
            v: 要输出的内容
            is_error: 是否作为错误输出，默认为True。
                如果为True，内容将作为错误输出，可能在前端显示为红色
                如果为False，内容将作为正常输出，在前端正常显示
        
        Returns:
            格式化的输出字典
        """
        prefix = "**ERROR**: " if is_error else ""
        return {"content": f"{prefix}{v}", "reference": []}
