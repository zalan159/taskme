...
def set_graph(tenant_id, kb_id, graph, docids):
    ...
    chunk = {
        ...
        "available_int": 1,  # 将available_int设置为1，表示所有用户可用
        "removed_kwd": "N"
    }
    
    # 检查是否已存在图数据
    print(f"Saving knowledge graph for kb {kb_id} with {len(graph.nodes())} nodes and {len(graph.edges())} edges")
    ...
    if res.ids:
        try:
            settings.docStoreConn.update({"knowledge_graph_kwd": "graph"}, chunk,
                                      search.index_name(tenant_id), kb_id)
            print(f"Updated existing knowledge graph for kb {kb_id}")
        except Exception as e:
            print(f"Error updating knowledge graph: {e}")
    else:
        try:
            settings.docStoreConn.insert([{"id": chunk_id(chunk), **chunk}], search.index_name(tenant_id), kb_id)
            print(f"Inserted new knowledge graph for kb {kb_id}")
        except Exception as e:
            print(f"Error inserting knowledge graph: {e}")
